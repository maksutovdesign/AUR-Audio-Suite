...
void Kit9Processor::prepareToPlay (double sr, int)
{
...
bool Kit9Processor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    const auto& out = layouts.getMainOutputChannelSet();
    return out == juce::AudioChannelSet::mono() || out == juce::AudioChannelSet::stereo();
}

void Kit9Processor::triggerVoice (int voice, float velocity)
{
    switch (voice)
    {
...
float Kit9Processor::renderVoiceSample (int voice)
{
    switch (voice)
    {
...
void Kit9Processor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
...
juce::AudioProcessorEditor* Kit9Processor::createEditor() { return new Kit9Editor (*this); }

int Kit9Processor::getNumPrograms() { return (int) Presets::getFactoryPresets().size(); }

void Kit9Processor::setCurrentProgram (int index)
{
...
const juce::String Kit9Processor::getProgramName (int index)
{
    const auto& p = Presets::getFactoryPresets();
    return (index >= 0 && index < (int) p.size()) ? p[(size_t) index].name : juce::String{};
}

void Kit9Processor::getStateInformation (juce::MemoryBlock& dest)
{
...
void Kit9Processor::setStateInformation (const void* data, int size)
{
    auto tree = juce::ValueTree::readFromData (data, (size_t) size);
    if (tree.isValid()) apvts.replaceState (tree);
}

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter() { return new Kit9Processor(); }
