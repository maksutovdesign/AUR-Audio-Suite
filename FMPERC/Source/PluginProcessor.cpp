...
FmpercProcessor::FmpercProcessor()
...
void FmpercProcessor::prepareToPlay (double sr, int)
{
    kick.prepare (sr); snare.prepare (sr); clap.prepare (sr);
    chat.prepare (sr); ohat.prepare (sr); tom.prepare (sr); rim.prepare (sr);
    // Per-pad FM character: carHz, ratio, index, decayMul, modEnvRel.
    kick.configure  (55.f,  0.5f,  5.f, 1.4f, 0.15f);
    snare.configure (220.f, 1.63f, 6.f, 0.7f, 0.5f);
    clap.configure  (900.f, 1.21f, 8.f, 0.5f, 0.9f);
    chat.configure  (700.f, 4.73f, 9.f, 0.35f, 1.0f);
    ohat.configure  (700.f, 4.73f, 9.f, 1.0f, 1.0f);
    tom.configure   (110.f, 1.4f,  3.f, 1.0f, 0.3f);
    rim.configure   (480.f, 2.76f, 7.f, 0.25f, 0.8f);
    drive.prepare (sr);
    drive.setFlavor (aur::ADAASaturator::Flavor::Tube);
    volGain.reset (sr, 0.02);
}

bool FmpercProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    const auto& out = layouts.getMainOutputChannelSet();
    return out == juce::AudioChannelSet::mono() || out == juce::AudioChannelSet::stereo();
}

void FmpercProcessor::triggerVoice (int voice, float velocity)
{
    switch (voice)
    {
...
float FmpercProcessor::renderVoiceSample (int voice)
{
    switch (voice)
    {
...
void FmpercProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
...
juce::AudioProcessorEditor* FmpercProcessor::createEditor() { return new FmpercEditor (*this); }

int FmpercProcessor::getNumPrograms() { return (int) Presets::getFactoryPresets().size(); }

void FmpercProcessor::setCurrentProgram (int index)
{
...
const juce::String FmpercProcessor::getProgramName (int index)
{
    const auto& p = Presets::getFactoryPresets();
    return (index >= 0 && index < (int) p.size()) ? p[(size_t) index].name : juce::String{};
}

void FmpercProcessor::getStateInformation (juce::MemoryBlock& dest)
{
...
void FmpercProcessor::setStateInformation (const void* data, int size)
{
    auto tree = juce::ValueTree::readFromData (data, (size_t) size);
    if (tree.isValid()) apvts.replaceState (tree);
}

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter() { return new FmpercProcessor(); }
