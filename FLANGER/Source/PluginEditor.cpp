...
FlangerEditor::FlangerEditor(FlangerProcessor& p):AudioProcessorEditor(p),ap(p){ setLookAndFeel(&lnf); auto& a=ap.getAPVTS();
...
FlangerEditor::~FlangerEditor(){ setLookAndFeel(nullptr);}
void FlangerEditor::applyThemeChoice(int i){ switch(i){case 1: aur::ui::setTheme(aur::ui::obsidianTheme());break; case 2: aur::ui::setTheme(aur::ui::fluxTheme());break; default: aur::ui::setTheme(aur::ui::moltenTheme());} lnf.applyTheme(); sendLookAndFeelChange(); repaint(); }
void FlangerEditor::refreshPresetBox(){ presetBox.clear(juce::dontSendNotification); int id=1; for(const auto& pr:Presets::getFactoryPresets()) presetBox.addItem(pr.name,id++); presetBox.setSelectedId(ap.getCurrentProgram()+1,juce::dontSendNotification);}
void FlangerEditor::paint(juce::Graphics& g){ const auto& t=theme(); g.fillAll(t.ground);
  ...
  drawBrandHeader(g,{20,16,360,60},"FLANGER","Stereo flanger"); }
void FlangerEditor::resized(){ auto area=getLocalBounds().reduced(18); auto header=area.removeFromTop(56); header.removeFromLeft(180);
