      ...
      apvts (*this, nullptr, "PARAMETERS", Params::createLayout())
{
    ...
    for (int i = 0; i < 8; ++i) synth.addVoice (new MorphVoice (params));
    synth.setNoteStealingEnabled (true);
}

void MorphProcessor::prepareToPlay (double sr, int)
{
    synth.setCurrentPlaybackSampleRate (sr);
    for (int i = 0; i < synth.getNumVoices(); ++i)
        if (auto* v = dynamic_cast<MorphVoice*> (synth.getVoice (i))) v->prepareVoice (sr);
...
int MorphProcessor::getNumPrograms() { return (int) Presets::getFactoryPresets().size(); }
void MorphProcessor::setCurrentProgram (int i) { if (i < 0 || i >= getNumPrograms()) return; currentProgram = i; Presets::apply (i, apvts); }
const juce::String MorphProcessor::getProgramName (int i) { const auto& p = Presets::getFactoryPresets(); return (i >= 0 && i < (int) p.size()) ? p[(size_t) i].name : juce::String{}; }
void MorphProcessor::getStateInformation (juce::MemoryBlock& d) { if (auto s = apvts.copyState(); s.isValid()) { juce::MemoryOutputStream mos (d, false); s.writeToStream (mos); } }
void MorphProcessor::setStateInformation (const void* d, int s) { auto t = juce::ValueTree::readFromData (d, (size_t) s); if (t.isValid()) apvts.replaceState (t); }
