...
class CrushEditor : public juce::AudioProcessorEditor{
public:
  explicit CrushEditor(CrushProcessor&); ~CrushEditor() override;
  void paint(juce::Graphics&) override; void resized() override;
private:
  void refreshPresetBox(); void applyThemeChoice(int);
  CrushProcessor& ap; aur::ui::AurLookAndFeel lnf;
  ...
  JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CrushEditor)
};
