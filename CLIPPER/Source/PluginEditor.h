...
public:
  explicit ClipperEditor(ClipperProcessor&); ~ClipperEditor() override;
  void paint(juce::Graphics&) override; void resized() override;
private:
  void refreshPresetBox(); void applyThemeChoice(int);
  ClipperProcessor& ap; aur::ui::AurLookAndFeel lnf;
  ...
  JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(ClipperEditor)
};
