...
bool BeatProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    const auto& out = layouts.getMainOutputChannelSet();
    return out == juce::AudioChannelSet::mono() || out == juce::AudioChannelSet::stereo();
}

void BeatProcessor::triggerVoice (int voice, float velocity)
{
    switch (voice)
    {
...
float BeatProcessor::renderVoiceSample (int voice)
{
    switch (voice)
    {
...
void BeatProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    // Host-synced 16-step sequencer (16th notes).
    if (*pSeqOn > 0.5f)
...
juce::AudioProcessorEditor* BeatProcessor::createEditor() { return new BeatEditor (*this); }

int BeatProcessor::getNumPrograms() { return (int) Presets::getFactoryPresets().size(); }

void BeatProcessor::setCurrentProgram (int index)
{
...
const juce::String BeatProcessor::getProgramName (int index)
{
    const auto& p = Presets::getFactoryPresets();
    return (index >= 0 && index < (int) p.size()) ? p[(size_t) index].name : juce::String{};
}

void BeatProcessor::getStateInformation (juce::MemoryBlock& dest)
{
...
void BeatProcessor::setStateInformation (const void* data, int size)
{
    auto tree = juce::ValueTree::readFromData (data, (size_t) size);
    if (tree.isValid()) apvts.replaceState (tree);
}

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter() { return new BeatProcessor(); }
